...
int	handle_pipe(t_ast_node *ast_head, int in_fd, int out_fd)
{
	int pipe_fd[2];
	pid_t left_pid;
	pid_t right_pid;
	...
	if (left_pid == 0)
	{
		...
		close(pipe_fd[1]);
		status = execute(ast_head->data.binary_op.left, in_fd, -1);
	...
	if (right_pid == 0)
	{
		...
		close(pipe_fd[0]);
		status = execute(ast_head->data.binary_op.right, -1, out_fd);
...
int handle_subshell(t_ast_node *ast_head, int in_fd, int out_fd)
{
	pid_t fork_pid;
	int status;
	...
	if (fork_pid == 0)
	{
		...
		status = execute(ast_head->data.sub_shell.subshell, -1, -1);
		exit(status);
	}
...
int	execute(t_ast_node *ast_head, int in_fd, int out_fd)
{
	...
	{
		return (execute_cmd(ast_head, in_fd, out_fd));
	}

...
t_token_lst	*handle_parentheses(t_token_lst *token_lst, t_ast_node **ast_node, int *last_status)
{
	token_lst = token_lst->next;
	token_lst = parse_or(token_lst, ast_node, last_status);
...
t_token_lst	*populate_command_data(t_token_lst *token_lst, t_ast_node **ast_node, int *last_status)
{
	...
	if (!(*ast_node)->data.cmd.executable)
	{
		...
		set_and_move_eight_bits_left(last_status, 127);
		return (NULL); // handle error
	}
...
t_token_lst	*parse_word(t_token_lst *token_lst, t_ast_node **ast_node, int *last_status)
{
	if (token_lst && (token_lst->type == TOKEN_WORD || is_redirection(token_lst->type)))
	{
		*ast_node = create_cmd_node(NODE_CMD, NULL, NULL, NULL); // handle if failure happens
		...
		if (!token_lst)
			return (NULL);
		token_lst = populate_command_data(token_lst, ast_node, last_status);
		token_lst = append_redirections_if_any(token_lst, ast_node);
		return (token_lst);
	}
	if (token_lst && token_lst->type == TOKEN_L_PAREN)
	{
		token_lst = handle_parentheses(token_lst, ast_node, last_status);
	}
	return (token_lst);
}
